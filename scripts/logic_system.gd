...
func _build_position() -> Vector3:
    ...
    if not hit.is_empty():
        return Vector3(hit.position) + Vector3(hit.normal) * 0.08
...
func _connect_nearest_pair() -> void:
    if int(player.components.get("cable",0)) < 1:
        ...
        return
    var source: LogicSource = _nearest_source(player.global_position, 14.0)
    var receiver: Node3D = _nearest_receiver(player.global_position, 14.0)
...
func _nearest_source(pos: Vector3, max_distance: float) -> LogicSource:
    ...
    for raw_node: Node in get_tree().get_nodes_in_group("logic_sources"):
        var source: LogicSource = raw_node as LogicSource
        if not is_instance_valid(source):
            continue
        var distance: float = source.global_position.distance_to(pos)
...
func _nearest_receiver(pos: Vector3, max_distance: float) -> Node3D:
    var best: Node3D
    var best_distance: float = max_distance
    for raw_node: Node in get_tree().get_nodes_in_group("logic_receivers"):
        var receiver: Node3D = raw_node as Node3D
        if not is_instance_valid(receiver) or not receiver.has_method("set_signal"):
            continue
        var distance: float = receiver.global_position.distance_to(pos)
        if distance < best_distance:
            best_distance = distance
            best = receiver
    return best
...
func _make_signal_cable(source: LogicSource, receiver: Node3D) -> void:
    var a: Vector3 = source.global_position + Vector3(0,0.45,0)
    var b: Vector3 = receiver.global_position + Vector3(0,0.45,0)
    var delta_position: Vector3 = b - a
    if delta_position.length() < 0.05:
        return
    ...
    mesh.height = delta_position.length()
