...
- All Unreal Python world-generation rotations use explicit roll/pitch/yaw keyword fields. A repository AST audit runs in CI and rejects positional unreal.Rotator calls so C++/Python constructor-order confusion cannot silently re-enter the project.
- The final Unreal bootstrap audit runs after every generation/staging pass, derives humanoid mesh foot offsets from real SkeletalMesh bounds, keeps grounded gameplay actors upright, normalizes known horizontal cylinder props and grounds staged gameplay actors against the actual curated floor elevations instead of one global guessed Z.
