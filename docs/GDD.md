...
- Unreal character setup now requires exact Skeleton identity between the runtime SkeletalMesh and every single-node animation clip; the Female Mannequin keeps its own Skeleton and remains a retarget target instead of being force-bound to the UAL1 animation Skeleton.
- Unreal transform/grounding audit is implemented but remains pending local visual validation. Python world-generation rotations use explicit roll/pitch/yaw semantics, final map sanitation runs after every staging pass, and an AST source audit at the end of the Unreal bootstrap rejects positional unreal.Rotator, rw.rotator and local rotator-helper calls.
- The FAS wheel pipeline now normalizes Engine Cylinder wheels to a local-Y axle after ConfigurePart so the visual axle matches ActorRightVector motor torque; the engineering proving ground keeps wheel Actor transforms neutral to avoid double rotation.
- The final Unreal bootstrap audit derives humanoid mesh foot offsets from real SkeletalMesh bounds, keeps grounded gameplay actors upright, normalizes known horizontal cylinder props and grounds staged gameplay actors against the actual curated floor elevations instead of one global guessed Z.
- The final audit deliberately does not perform blanket legacy Euler remapping; idempotent art/dressing/weather passes rebuild their generated actors from corrected source so freshly-correct rotations cannot be accidentally transformed a second time.
...
- Humanoid enemies use a reusable Skeleton-native animation family with sight/hearing/flashlight awareness; runtime NPC code rejects mismatched animation Skeletons instead of allowing a broken T-pose fallback, while a rigid-part procedural crawler provides a production-efficient non-humanoid threat.
- The mannequin-scale Colossus uses the same Skeleton-compatibility rule, computes its visual floor alignment from SkeletalMesh bounds, grounds its capsule to the world and keeps weakpoint collision bands aligned with the giant body while retaining harpoon support and environmental trampling.
